...
export default async function ProductPage({ params }) {
  ...
  const images = [p.main_image, ...gallery].filter(
    (u) => typeof u === "string" && (u.startsWith("/i/") || u.startsWith("http"))
  );
  ...
  return (
    <div className="container mx-auto px-4 py-10 grid grid-cols-1 md:grid-cols-2 gap-8">
      ...
      <div className="flex flex-col gap-3">
        ...
        <AddToCart product={{ slug: p.slug, name: p.name, price: p.price, images: images.length ? images : ["/placeholder.png"], sizes, colors, quantity: p.quantity }} />
      </div>
    </div>
  );
}
