...
export default async function ProductPage({ params }) {
  const p = await first("SELECT * FROM products WHERE slug=?", params.slug);
  ...
  return (
    <div className="container mx-auto px-4 py-10 grid grid-cols-1 md:grid-cols-2 gap-8">
      <div>
        <img src={images[0]} alt={p.name} className="w-full aspect-[3/4] object-cover border" />
      </div>
      <div className="flex flex-col gap-3">
        ...
        <AddToCart product={{ slug:p.slug, name:p.name, price:p.price, images, sizes, colors }} />
      </div>
    </div>
  );
}
